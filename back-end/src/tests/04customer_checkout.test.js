...
describe("Testes na rota /sale", () => {
  describe("Testes com POST", () => {
    it("Deve criar uma nova venda na tabela sales", async () => {
      ...
      const response = await chai.request(app).post('/sale').set('authorization', login.body.token).send(mockInputSale);
    ...
    it("Deve falhar ao tentar cadastrar uma venda com um token inválido", async () => {
      ...
      const response = await chai.request(app).post('/sale').set('authorization', 'token').send(mockInputSale);
    ...
    it("Deve falhar ao tentar cadastrar uma venda sem um token", async () => {
      ...
      const response = await chai.request(app).post('/sale').send(mockInputSale);
